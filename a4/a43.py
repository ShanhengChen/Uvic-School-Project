...
'''
instantiated to create different art types
'''
class ArtConfig:
    def __init__(self, f: IO[str], t: int):
        self.shape_num =  random.randint(5000,10000)
        self.f = f 
        self.t = t
    '''
    Determine the type of graphics and generate shapes corresponding to the relevant type data from GenRandom object (10,000 times)
    '''
    def handleRandom(self):
        self.shape = GenRandom()
        while self.shape_num > 0:
            if self.shape.SHA == 0:
               drawCircleLine(self.f, self.t, Circle((self.shape.X,self.shape.Y,self.shape.RAD), (self.shape.R,self.shape.G,self.shape.B,self.shape.OP))) 
            if self.shape.SHA == 1:
                drawRectangleLine(self.f, self.t, Rectangle((self.shape.X,self.shape.Y,self.shape.W,self.shape.H), (self.shape.R,self.shape.G,self.shape.B,self.shape.OP)))
            if self.shape.SHA == 3:
                drawProEpilogueLine(self.f, self.t, ProEpilogue((self.shape.X,self.shape.Y,self.shape.RX,self.shape.RY), (self.shape.R,self.shape.G,self.shape.B,self.shape.OP)))
            self.shape_num -= 1
            self.shape = GenRandom()
